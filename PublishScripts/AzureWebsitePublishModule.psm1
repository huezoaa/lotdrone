...
<#
.SYNOPSIS
Prepends the date and time to a Notification.

.DESCRIPTION
Prepends the date and time to a Notification. This function is designed for Notifications written to the Error and Verbose streams.

.PARAMETER  Notification
Specifies the Notifications without the date.
...
.EXAMPLE
PS C:\> Format-DevTestNotificationWithTime -Notification "Adding file $filename to the directory"
...
Write-ErrorWithTime
#>
function Format-DevTestNotificationWithTime
{
    [CmdletBinding()]
    param
    (
    ...
    return ((Get-Date -Format G)  + ' - ' + $Notification)
}
...
.SYNOPSIS
Writes an error Notification prefixed with the current time.

.DESCRIPTION
Writes an error Notification prefixed with the current time. This function calls the Format-DevTestNotificationWithTime function to prepend the time before writing the Notification to the Error stream.

.PARAMETER  Notification
Specifies the Notification in the error Notification call. You can pipe the Notification string to the function.
...
.EXAMPLE
PS C:> Write-ErrorWithTime -Notification "Failed. Cannot find the file."
...
function Write-ErrorWithTime
{
    ...
    $Notification | Format-DevTestNotificationWithTime | Write-Error
}


<#
.SYNOPSIS
Writes a verbose Notification prefixed with the current time.

.DESCRIPTION
Writes a verbose Notification prefixed with the current time. Because it calls Write-Verbose, the Notification displays only when the script runs with the Verbose parameter or when the VerbosePreference preference is set to Continue.

.PARAMETER  Notification
Specifies the Notification in the verbose Notification call. You can pipe the Notification string to the function.
...
.EXAMPLE
PS C:> Write-VerboseWithTime -Notification "The operation succeeded."
PS C:>
PS C:\> Write-VerboseWithTime -Notification "The operation succeeded." -Verbose
...
function Write-VerboseWithTime
{
    ...
    $Notification | Format-DevTestNotificationWithTime | Write-Verbose
}


<#
.SYNOPSIS
Writes a host Notification prefixed with the current time.

.DESCRIPTION
This function writes a Notification to the host program (Write-Host) prefixed with the current time. The effect of writing to the host program varies. Most programs that host Windows PowerShell write these Notifications to standard output.

.PARAMETER  Notification
Specifies the base Notification without the date. You can pipe the Notification string to the function.

.INPUTS
System.String

.OUTPUTS
None. The function writes the Notification to the host program.

.EXAMPLE
PS C:> Write-HostWithTime -Notification "The operation succeeded."
...
function Write-HostWithTime
{
    ...
    if ((Get-Variable SendHostNotificationsToOutput -Scope Global -ErrorAction SilentlyContinue) -and $Global:SendHostNotificationsToOutput)
    {
        ...
        $Global:AzureWebAppPublishOutput += $Notification | Format-DevTestNotificationWithTime
    }
    else 
    {
        $Notification | Format-DevTestNotificationWithTime | Write-Host
    }
}
